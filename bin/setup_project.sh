...
echo "Changing references of 'python_skeleton' and 'fdm1' to '$PROJECT_NAME' and '$GIT_USERNAME'"
# Change references in files
for file in ${files_to_change[@]}; do
   perl -i -pe "s/python_skeleton/$PROJECT_NAME/g" $file
   perl -i -pe "s/fdm1/$GIT_USERNAME/g" $file
done

# Rename main library directory
mv python_skeleton $PROJECT_NAME


echo "Clearing out the README"
# Probably more elegant way to do this.
perl -i -pe "s/Python Skeleton/$PROJECT_NAME/g" README.md
